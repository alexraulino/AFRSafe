...
public class RegisterActivity extends Activity {
	...
	private void registerUser(final String name, final String email,
		 ...
		 * "Register Response: " + response.toString()); hideDialog();
		 * 
		 * try { JSONObject jObj = new JSONObject(response); boolean error =
		 * jObj.getBoolean("error"); if (!error) { // User successfully stored
		 * in MySQL // Now store the user in sqlite String uid =
		 * jObj.getString("uid");
		 * 
		 ...
		 * RegisterActivity.this, LoginActivity.class); startActivity(intent);
		 * finish(); } else {
		 * 
		 * // Error occurred in registration. Get the error // message String
		 * errorMsg = jObj.getString("error_msg");
		 * Toast.makeText(getApplicationContext(), errorMsg,
		 * Toast.LENGTH_LONG).show(); } } catch (JSONException e) {
		 * e.printStackTrace(); }
		 * 
		 ...
		 * params to register url Map<String, String> params = new
		 * HashMap<String, String>(); params.put("name", name);
